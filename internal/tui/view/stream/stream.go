...
type model struct {
	contents    []aws.LogStream
	headerTitle string
	ready       bool
	viewport    viewport.Model
}

type openLogStreamMsg struct{}
type openLogGroupMsg []aws.LogStream
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case aws.LogStreamsMsg:
		m.headerTitle = "Logs"

		messages := []string{}
		for _, s := range msg {
			messages = append(messages, s.Contents)
		}

		m.viewport.SetContent(strings.Join(messages, "\n"))
		cmds = append(cmds, view.SetView(view.Stream))
	}
